<template>
  <div class="board">
    ...
    <div class="buttons">
      <div class="left">
        <button class="button" v-if="isEditable" @click="boardEditClick">
          수정
        </button>
        <button class="button" v-if="isEditable" @click="boardDeleteClick">
          삭제
        </button>
      </div>
      <div class="right">
...
export default {
  ...
  methods: {
    getBoardRead() {
      this.axios
        .get("http://localhost:9000/boards/" + this.$route.query.boardNo)
        .then((res) => {
          console.log(res);
    ...
    boardEditClick() {
      ...
      if (result) {
        this.$router.push({
          name: "BoardEdit",
          query: { boardNo: this.boardItem.no },
        });
      }
    },
      ...
      if (result) {
        try {
          let res = await this.axios.delete(
            "http://localhost:9000/boards/" + this.$route.query.boardNo
          );
