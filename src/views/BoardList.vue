<template>
  <div class="board">
    <h1>This is board list page</h1>
    <div class="buttons">
      <div class="right">
        <router-link to="/board/boardwrite" class="button blue"
          ><span>쓰기</span></router-link
        >
        <!-- <router-link :to="{ name: 'BoardWrite' }" class="button blue" 
          ><span>쓰기</span></router-link
        >-->
      </div>
    </div>
    <table>
      ...
      <tbody>
        <tr v-for="boardItem in boardList" v-bind:key="boardItem.no">
          <td>
            <span @click="boardNoClick(boardItem)">{{ boardItem.no }}</span>
          </td>
          <td>
            <router-link
              :to="{ name: 'BoardView', query: { boardNo: boardItem.no } }"
              >{{ boardItem.subject }}</router-link
            >
          </td>
